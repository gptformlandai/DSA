...
## Pattern 4: Remove Nth Node from End (One Pass)

**Trick:** Move fast pointer n steps ahead. When fast reaches null, slow is at the node to remove.
...
```
fast moves 3 steps (n+1=3): fast at node 3
Then both move:
fast=4,slow=2 → fast=5,slow=3 → fast=null,slow=4
...
## Pattern 6: Reorder List (L0→Ln→L1→Ln-1→...)
...
## Pattern 7: Clone List with Random Pointer

**Trick:** Insert clones between original nodes, set random pointers, then detach.

```java
Node copyRandomList(Node head) {
    if (head == null) return null;
    // Step 1: Insert clone after each node
    Node curr = head;
    while (curr != null) {
...
## Pattern 8: LRU Cache (HashMap + Doubly Linked List)
