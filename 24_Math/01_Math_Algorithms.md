...
long lcm(long a, long b) {
    return a / gcd((int)a, (int)b) * b;  // divide first to avoid overflow
}
```

**Why it works:** GCD(a, b) = GCD(b, a mod b). The remainder always decreases → terminates.
...
Why only check up to √n? If n has a factor > √n, its pair is < √n (already checked).
...
boolean[] sieve(int n) {
    ...
    return isComposite;  // !isComposite[i] means i is prime
}
```
...
// Subtraction (avoid negative)
((a - b) % MOD + MOD) % MOD

// Multiplication
(a * b) % MOD

// Division (requires modular inverse — only when MOD is prime)
// a/b mod MOD = a * modInverse(b, MOD) % MOD
```

---

## 4. Fast Exponentiation — O(log n)
...
**Example:** 2^10 = 2^8 × 2^2 = 256 × 4 = 1024 (only 4 multiplications)
...
long modInverse(long a, long mod) {
    return power(a, mod - 2, mod);  // Fermat's little theorem (mod must be prime)
}
```

---

## 6. Useful Math Tricks
...
// Count up to √n, each pair gives two factors

// Integer overflow guard
(long)a * b > c  // cast before multiply
```
...
**Easy:**
1. Palindrome Number.
2. Power of Three / Power of Two.
3. Fibonacci Number.

**Medium:**
1. Count Primes (Sieve).
2. Happy Number.
3. Pow(x, n) — fast exponentiation.
4. Excel Sheet Column Number.
5. Find the Difference (XOR math).

**Hard:**
1. Largest Number (comparison-based greedy).
2. Ugly Number III.
