...
**Overlap condition:** interval A [a1,a2] and B [b1,b2] overlap if `b1 <= a2` (B starts before A ends).
...
## Pattern 3: Non-Overlapping Intervals (Minimum Removals)
...
int eraseOverlapIntervals(int[][] intervals) {
    ...
    for (int[] interval : intervals) {
        if (interval[0] >= end) end = interval[1];  // no overlap, keep
        else removals++;                             // overlap, remove (greedy: keep earlier end)
    }
    return removals;
}
```
...
**How many rooms needed to host all meetings simultaneously?**
...
int minMeetingRooms(int[][] intervals) {
    ...
    for (int start : starts) {
        if (start < ends[j]) rooms++;   // new meeting starts before any ends
        else j++;                        // a meeting ends, room freed
...
start=15: 15>10 → j=1 (end=20), rooms still 2? No: 15<20 → rooms=3? 
Wait: 15<20 so rooms++ → rooms=3? Let me recheck...
Actually: room freed when start >= end[j].
start=15 >= end[0]=10 → j=1, don't increment rooms (room reused)
Result: 2 rooms needed
```

---

## Pattern 5: Sweep Line for Intervals

Count maximum overlap at any point:
```java
int maxOverlap(int[][] intervals) {
    Map<Integer, Integer> diff = new TreeMap<>();
    for (int[] interval : intervals) {
        diff.merge(interval[0], 1, Integer::sum);
        diff.merge(interval[1], -1, Integer::sum);
    }
...
## Pattern 6: Difference Array for Range Updates

(Covered in depth in `07_Prefix_Sum/01_Prefix_Sum.md`)
...
**Easy:**
1. Meeting Rooms I (can one person attend all?).
2. Summary Ranges.
3. Missing Ranges.
